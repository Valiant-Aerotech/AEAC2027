...
| **Tools** | `tools/valiant.py` CLI; subfolders `bench/`, `gcs/`, `sitl/`, `bringup/`, `calibrate/`, `deploy/`, `dev/`; frequent PS1 stay at `tools/` root |
| **SITL YAML missions** | `config/sitl_missions/*.yaml` + `python tools/valiant.py sitl run …` for geofence, tuning, and scenario without editing `vion.yaml` |
