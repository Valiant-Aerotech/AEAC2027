...
from valiant.perception.detect.constants import CAPTURE_HEIGHT, CAPTURE_WIDTH
...
class YoloOnnxDetector:
    ...
    def __init__(self, model_path: Path, *, conf_thresh: float = 0.35, label: str = ""):
    ...
    def detect(self, frame: npt.NDArray[np.uint8]) -> DetectionFrame:
        """Single pass over a centre crop; at most one detection."""
        h, w = frame.shape[:2]
        best = self.detect_best(frame)
        return DetectionFrame(
            detections=[best] if best is not None else [],
            frame_w=w,
            frame_h=h,
            source="yolo_onnx",
        )

    def detect_best(self, frame: npt.NDArray[np.uint8]) -> Detection | None:
