"""Tiled YOLO detection over a large frame.

The frame is cut into a grid of fixed-size tiles and inference runs per tile in
a spiral from the centre outward, stopping after a budget of tiles. This finds
objects that are only a handful of pixels across in the full image - which is
the whole problem for the 2027 survey, where a deer at 100 m AGL and a 3 cm ear
tag both vanish under a single full-frame resize.

Detections that land on a tile edge trigger inference on the neighbouring tile,
so an object straddling a seam is not cut in half.
"""
...
def resolve_model_path(cfg: dict) -> Path | None:
    """First existing weights file from config, then conventional locations."""
    cv_cfg = cfg.get("cv", {})
    model_rel = cv_cfg.get("models", {}).get("primary", "models/detect.onnx")
    candidates = [
        repo_root() / model_rel,
        repo_root() / "models" / "detect.onnx",
        repo_root() / "models" / "detect.pt",
    ]
    seen: set[Path] = set()
...
class SubframeYoloDetector(Degradable):
    ...
    def __init__(self, cfg: dict):
        ...
        # Maps model class ids to mission labels, e.g. {0: "deer", 1: "ear_tag"}.
        self._labels: dict[int, str] = {
            int(k): str(v) for k, v in cfg.get("cv", {}).get("labels", {}).items()
        }
    ...
    def detect(self, frame: npt.NDArray[np.uint8]) -> DetectionFrame:
        ...
        return DetectionFrame(
            ...
            frame_w=w,
            frame_h=h,
            source="subframe_yolo",
        )

    def detect_all(self, frame: npt.NDArray[np.uint8]) -> list[Detection]:
        ...
        return [self._to_detection(d, frame.shape[1], frame.shape[0]) for d in final]
    ...
    def _to_detection(self, det: dict[str, Any], frame_w: int, frame_h: int) -> Detection:
        ...
        return Detection(
            ...
            label=self._labels.get(class_id, ""),
        )
