...
New-Issue "C10: Outdoor HSV tuning - dry and shot targets" "## Problem`nHSV thresholds in config/vion.yaml are tuned for synthetic images. Outdoor sun/shadow will cause missed detections and false positives.`n`n## Tasks`n- [ ] Print 5-30 cm purple competition circles`n- [ ] Run python tools/valiant.py bench cv --camera 0 at field site AM/PM lighting`n- [ ] Tune hsv_dry, hsv_shot, hsv_min_area_px`n- [ ] Document final values in runbook`n`n## Done when`n10 consecutive stable dry detections outdoors; shot detected within 3 s after wetting." @("track-c","cv","refinement","field-test","priority-high") $msC
