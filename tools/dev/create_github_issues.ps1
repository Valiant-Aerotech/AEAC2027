...
New-Issue "C13: CV public API + subframe YOLO" "## Done in AEAC2027 repo via pull request 85.`n`n- valiant.autonomy.cv facade (api.py, __init__.py)`n- Subframe spiral YOLO (subframe_yolo.py, subframe_grid.py, dry_detector.py)`n- hits_to_bench_dict for getTargets() parity`n- Import boundaries: test_subsystem_boundaries.py, test_cv_api.py`n- Dry=YOLO; shot=HSV until shot.onnx`n`nSee docs/interfaces.md CV public API section." @("track-c","done","cv") $msC -Close
New-Issue "C10: Outdoor HSV tuning - dry and shot targets" "## Problem`nHSV thresholds in config/vion.yaml (inherited by config/rpas.yaml) are tuned for synthetic images. Outdoor sun/shadow will cause missed detections and false positives.`n`n## Tasks`n- [ ] Print 5-30 cm purple competition circles`n- [ ] Run python tools/valiant.py bench cv --camera 0 at field site AM/PM lighting`n- [ ] Tune hsv_dry, hsv_shot, hsv_min_area_px`n- [ ] Document final values in runbook`n`n## Done when`n10 consecutive stable dry detections outdoors; shot detected within 3 s after wetting." @("track-c","cv","refinement","field-test","priority-high") $msC
New-Issue "C11: CV regression set from recorded footage" "## Goal`nRecord scrcpy footage at field tests; replay through valiant bench cv for regression.`n`n## Command`npython tools/valiant.py bench cv --regression --video path/to/clip.mp4`n`n## Done when`nAt least 3 clips saved in team Drive; script logs false positive/negative counts per clip." @("track-c","cv","refinement","priority-medium") $msC
...
New-Issue "D11: VL53L1X rangefinder field validation" "## Goal`nValidate MAVLink DISTANCE_SENSOR on Vion FC.`n`n## Tasks`n- [ ] Set metric_recon.rangefinder: vl53l1x`n- [ ] Run python tools/valiant.py bench metric --camera 0 with live link`n- [ ] Compare to tape measure at 1, 2, 3 m`n`n## Done when`nDistance within 15% of truth and planner 2 m gate works with rangefinder not FOV-only." @("track-d","metric-recon","field-test","priority-high") $msD
...
New-Issue "D13: Auto-nav PD gain field tuning" "## Problem`nBench PD gains may oscillate or creep on real Vion.`n`n## Tasks`n- [ ] Phase 2 tests 2.2-2.3`n- [ ] Tune kp_x/y, deadband_px, approach_speed in config/vion.yaml (airframe tuning)`n`n## Done when`nTarget holds centre within deadband for 5 s without pilot override." @("track-d","auto-nav","field-test","refinement","priority-high") $msD
New-Issue "D14: Side clearance calibration" "## Goal`nValidate planner behaviour against real door frames and wall edges.`n`n## Repo`nEdge clearance code on main via pull request 85. SITL and unit tests pass.`n`n## Still open`nField drills 2.4 and 2.4b-d in docs/runbooks/field-test-plan.md." @("track-d","auto-nav","field-test","priority-medium") $msD
New-Issue "D15: Edge clearance avoidance" "## Done in AEAC2027 repo via pull request 85`n`nLeft/right/top/bottom edge proximity, virtual aim, planner bypass, dual spray alignment.`n`n- edge_proximity.py, aim_offset.py, lateral/vertical_clearance.py`n- MetricPacket edge fields + spray/aim.py dual alignment`n- HUD edge labels; SITL ceiling constraint`n`nField calibration tracked on D14." @("track-d","done","metric-recon","auto-nav","spray") $msD -Close
New-Issue "D16: Subsystem public API facades" "## Done in AEAC2027 repo via pull request 85`n`n- metric_recon/api.py, auto_nav/api.py, spray/api.py`n- Orchestrator imports public APIs only`n- test_subsystem_boundaries.py, test_public_apis.py`n`nSee docs/interfaces.md public API sections." @("track-d","done","metric-recon","auto-nav","spray","infra") $msD -Close
...
New-Issue "E7: SITL Mission Planner STATUSTEXT (GCS HUD)" "## Done in repo`n`n- gcs_hud.py: human-readable T2: lines for flight line`n- mp_use_autopilot_sysid for MP Messages on tcp:5762`n- python tools/valiant.py gcs verify-statustext`n- config/vion.yaml gcs_monitor keys`n`nSee docs/runbooks/sitl-wsl.md Mission Planner section." @("track-e","done","infra") $msE -Close
New-Issue "E8: SITL guided box pattern flight" "## Done in repo`n`n- sitl_pattern.py: forward/turn legs, LOITER end state`n- Yaw via SET_POSITION_TARGET_LOCAL_NED mask 2503 at 20 Hz`n- python tools/valiant.py sitl pattern`n- tools/run_sitl_pattern.ps1`n`nSee docs/runbooks/sitl-wsl.md guided box pattern section." @("track-e","done","auto-nav","infra") $msE -Close
New-Issue "E9: Script path audit and WSL distro fixes" "## Done in repo`n`n- tools/lib/wsl_distro.ps1 repo root detection`n- tools/lib/diagnostics.ps1 import path`n- tools/dev/audit_script_paths.py in verify_ps1`n- python tools/valiant.py diagnose hints`n`nSee START_HERE.md and tools/README.md." @("track-e","done","infra") $msE -Close
New-Issue "E10: Vivi GUIDED orbit field test" "## Goal`nOutdoor Vivi + Pi: pilot selects GUIDED at 10 m; companion runs orbit then LOITER.`n`n## Repo status`n- field_orbit.py + guided_motion.py`n- python tools/valiant.py sitl orbit (SITL first)`n- python hardware/vion/rpi/run_field_orbit.py --profile vivi_orbit`n- docs/runbooks/vivi-orbit-field-test.md`n`n## Field pass`n5 laps R=5 m, return center, LOITER, manual RC home." @("track-e","field-test","auto-nav","priority-high") $msFT
...
# --- Epic ---
New-Issue "Roadmap: AEAC2027 field test phases 0-6" "## Master checklist`n`ndocs/runbooks/field-test-plan.md`n`ndocs/runbooks/competition-day.md`n`ndocs/github-issues-backlog.md`n`n## Whiteboard gaps`n- CV: outdoor HSV, shot ONNX, shot confirmation, regression clips`n- Metric Recon: VL53L1X and FOV calibration`n- Auto-Nav: PD tuning, side and edge clearance field tune`n- Upload: real Google Drive`n`n## Repo ready on main`n- CV public API and subframe YOLO`n- Edge clearance four edges`n- Subsystem public APIs`n`nClose sub-issues as each phase passes on hardware." @("field-test","priority-high") $msFT
